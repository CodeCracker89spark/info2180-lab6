...
<?php if(isset($_GET['query']) && !empty($_GET['query'])): ?>
    ...
    <?php foreach($superheroes as $sups): ?>
    <?php if(strcasecmp($sups['alias'], $_GET['query'])===0/*$sups['alias']===$_GET['query']*/ OR $sups['name']===$_GET['query']): ?>
        <?php $present = true;?>
     <h3 id = "cover" class = "details"><?= $sups['alias'] ?></h4>
     
    
    <h4 id = "identity" class = "details"> A.K.A <?= $sups['name']  ?></h4>
      <br>
      <p class = "details"><?= $sups['biography'] ?></p>
    <?php endif; ?>
    <?php endforeach; ?>
<?php endif; ?>
<?php if(isset($_GET['query']) && empty($_GET['query'])): ?>
...
<?php if($present === false and  !empty($_GET['query'])): ?>   
    <h3 id = "absent"><?= "SUPERHERO NOT FOUND" ?></h3>
  <?php endif; ?>
